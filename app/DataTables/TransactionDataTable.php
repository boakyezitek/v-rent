...
class TransactionDataTable extends DataTable
{
    ...
    public function query()
    {
        ...
        })->select(['withdrawals.id as id',  'withdrawals.amount as amount', 'withdrawals.currency_id AS c_id', 'currency.symbol AS symbol', 'currency.code AS code', 'payment_methods.name AS p_method', 'withdrawals.created_at AS date', DB::raw('0 as type')])->where('withdrawals.user_id', '=', $user_id)->where('withdrawals.status', '=', 'Success');
        ...
        $trips = Bookings::join('currency', function ($join) {
            ...
            $join->on('bookings.payment_method_id', '=', 'payment_methods.id');
        })->select(['bookings.id as id', 'bookings.total AS amount', 'bookings.currency_code AS c', 'currency.symbol AS symbol', 'currency.code AS code','payment_methods.name AS p_method', 'bookings.created_at AS date', DB::raw('-1 as type')])->where('bookings.user_id', '=', $user_id)->where('bookings.status', '=', 'Accepted');
    ...
    public function html()
    {
        ...
        ->addColumn(['data' => 'p_method', 'name' => 'payment_methods.name', 'title' => trans('messages.utility.payment_method'),'searchable' => true])
